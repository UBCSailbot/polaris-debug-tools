...
class GraphObject: # struct which keeps together objects needed for a graph
    def __init__(self, x_name, y_name, x_units, y_units, init_min, init_max, minn, maxn): # data = history?
        '''
    ...
    def initialize(self):
        self.graph = create_graph(f"{self.x_name} vs. {self.y_name}", f"{self.x_name} ({self.x_units})" if self.x_units else f"{self.x_name}", f"{self.y_name} ({self.y_units})")
        self.graph.show()
